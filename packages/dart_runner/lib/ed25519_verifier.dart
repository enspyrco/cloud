...
Uint8List Hash(Uint8List m) => Digest('SHA-512').process(m);
...
Uint8List secretKey([int seed]) {
  Random randGen;
  if (seed == null) {
    randGen = Random.secure();
  } else {
    randGen = Random(seed);
  }
  var randList = List<int>.generate(1024, (_) => randGen.nextInt(bits));
...
Uint8List sign(Uint8List message, Uint8List secretKey, Uint8List pubKey) {
  ...
  var secretKeyAddMsg = List<int>.from(secretHash.sublist(32, 64));
...
bool verifySignature(Uint8List signature, Uint8List message, Uint8List pubKey) {
  ...
  var preDigest = List<int>.from(sigSublist);
  preDigest.addAll(pubKey);
  preDigest.addAll(message);
